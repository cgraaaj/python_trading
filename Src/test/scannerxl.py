...
async def test():
    ...
    while True:
        ...
        if total_count_limit != 0 and total_messages >= total_count_limit:
            ...
            all_sheet.range("A3").options(index=False,header=False).value = jkdiamond_df.drop(["signal","date"],axis=1)
            all_sheet.range("D3").options(index=False,header=False).value = premom_df.drop(["signal","date"],axis=1)
            all_sheet.range("G3").options(index=False,header=False).value = wpremom_df.drop(["signal","date"],axis=1)
            all_sheet.range("J3").options(index=False,header=False).value = weekly_df.drop(["signal","date"],axis=1)
            all_sheet.range("M3").options(index=False,header=False).value = monthly_df.drop(["signal","date"],axis=1)
            offset_id = 0
            all_messages = []
