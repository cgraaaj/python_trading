...
def get_data():
	...
	print(PE_otm_strike)
	df =  df[(df['tradingsymbol'] == CE_deep_itm_strike) | (df['tradingsymbol'] == PE_deep_itm_strike) ]
	...
	for i in range(0,len(trade_token)):
		...
		trade_token[i]
		try:
			df = pd.DataFrame(kite.historical_data(trade_token[i], curr_day+' 09:15:00', curr_day+' 15:25:00', '5minute'))
			df_prev = pd.DataFrame(kite.historical_data(trade_token[i], prev_day_dt+' 09:15:00', prev_day_dt+' 15:25:00', '5minute'))
			print(df)
			if not df_prev.empty:
				df = df[['date', 'open', 'high', 'low', 'close', 'volume','oi']]
				df['date'] = df['date'].astype(str).str[:-6]
				df_prev = df_prev[['date', 'open', 'high', 'low', 'close', 'volume','oi']]
		...
		# print(df)
		if not df.empty:
			df_prev['vwap']=(((df_prev['high']+df_prev['low']+df_prev['close'])/3)*df_prev['volume']).cumsum()/df_prev['volume'].cumsum()
			...
			df_full['volume_break'] = np.where((df_full['volume'] > df_full['vol_sma']), True, False)
			df_full['oi_break'] = np.where((df_full['oi'] < df_full['oi_sma']), True, False)
			df_full['vwap_break'] = np.where((df_full['vwap'] < df_full['close']), True, False)
			
			# print(df_full)
			df_full['long_positions'] = np.where(df_full['rsi_break'] & df_full['volume_break'] & df_full['oi_break'] & df_full['vwap_break'], 1, 0)
			print(df_full)
			print(df_full['date'].iloc[-1])
			if((str(df_full['long_positions'].iloc[-2])=='1')):
				message_buy="Buy "+str(tickerlist[i][-7:])+" at "+str(df_full['high'].iloc[-2])
		time.sleep(0.05)
