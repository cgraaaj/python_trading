...
class Tele:
    def __init__(self, chat_ids_fname, data_path):
        LOCATE_PY_DIRECTORY_PATH = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "..")
        )
        load_dotenv("{}/.env".format(LOCATE_PY_DIRECTORY_PATH))
        file1 = open(
            "{}/data/{}.txt".format(LOCATE_PY_DIRECTORY_PATH, chat_ids_fname), "r"
        )
        ...
        for chat_id in self.chat_ids:
            for file in os.listdir(self.data_path):
                self.bot.sendDocument(
                    chat_id=chat_id, document=open(f"{self.data_path}/{file}", "rb")
                )
